...
impl TestApp {
    pub fn get_confirmation_links(
        ...
        let get_link = |s: &str| {
            ...
            let mut conformation_link = reqwest::Url::parse(&raw_link).unwrap();

            assert_eq!(conformation_link.host_str().unwrap(), "127.0.0.1");
            conformation_link.set_port(Some(self.port)).unwrap();
            conformation_link
        };
...
pub fn assert_is_redirect_to(response: &reqwest::Response, location: &str) {
    dbg!(format!("Redirect to {}", location,));
